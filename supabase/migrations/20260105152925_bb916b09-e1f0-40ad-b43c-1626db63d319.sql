-- Drop all existing versions of search_blog_posts
...
-- Recreate the function with correct parameter order matching useBlogSearch hook
...
BEGIN
  ...
  WHERE
    ...
    AND (filter_site IS NULL OR bp.source_site = filter_site OR (bp.shared_sites IS NOT NULL AND filter_site = ANY(bp.shared_sites)))
    AND (

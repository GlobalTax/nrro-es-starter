/**
 * INTRANET DESIGN SYSTEM - Apollo.io-inspired Components
 */
...
// ============================================
// STATS CARD — Apollo inline icon style
// ============================================
...
export function AdminStatsCard({ label, value, icon, trend, className }: AdminStatsCardProps) {
  return (
    <div className={cn(
      "bg-white border border-gray-100 rounded-lg p-5 transition-colors hover:border-gray-200",
      className
    )}>
...
export function AdminCard({ children, className, padding = 'md', hover = false }: AdminCardProps) {
  ...
  return (
    <div className={cn(
      ...
      hover && "transition-colors hover:border-gray-200",
      className
    )}>
      {children}
    </div>
...
export function AdminBadge({ children, variant = 'default', size = 'sm', className }: AdminBadgeProps) {
  ...
  const sizeClasses = { sm: 'px-2 py-0.5 text-[11px]', md: 'px-2.5 py-1 text-xs' };
...
// ============================================
// TABLE — compact, no uppercase headers
// ============================================

interface AdminTableProps { children: ReactNode; className?: string; }
...
interface AdminTableHeaderProps { children: ReactNode; className?: string; }
...
interface AdminTableBodyProps { children: ReactNode; className?: string; }
export function AdminTableBody({ children, className }: AdminTableBodyProps) {
  return <tbody className={cn("", className)}>{children}</tbody>;
}

interface AdminTableRowProps { children: ReactNode; className?: string; onClick?: () => void; selected?: boolean; }
...
interface AdminTableHeadProps {
  ...
  sortable?: boolean; sorted?: 'asc' | 'desc' | null; onSort?: () => void;
}
export function AdminTableHead({ children, className, sortable, sorted, onSort }: AdminTableHeadProps) {
  return (
    <th 
      className={cn(
        "text-left py-2.5 px-4 text-[12px] font-medium text-gray-500",
...
interface AdminTableCellProps { children: ReactNode; className?: string; }
export function AdminTableCell({ children, className }: AdminTableCellProps) {
  return <td className={cn("py-2.5 px-4 text-gray-900", className)}>{children}</td>;
}
...
export function AdminDivider({ className }: { className?: string }) {
  return <hr className={cn("border-t border-gray-100 my-5", className)} />;
}
...
export function AdminEmptyState({ icon, title, description, action, className }: AdminEmptyStateProps) {
  return (
    <div className={cn("flex flex-col items-center justify-center py-12 text-center", className)}>
      ...
      {description && <p className="text-[13px] text-gray-500 mb-4 max-w-sm">{description}</p>}
      {action}
    </div>
  );
}
...
export function AdminSectionHeader({ title, description, actions, className }: AdminSectionHeaderProps) {
  return (

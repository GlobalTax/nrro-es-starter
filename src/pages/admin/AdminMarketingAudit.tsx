...
import { AuditScheduleManager } from '@/components/admin/marketing-audit/AuditScheduleManager';
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { ScanLine, Loader2, RotateCcw, Save, Calendar } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-medium text-white">Auditoría de Marketing Digital y SEO</h1>
          <p className="text-sm text-slate-400 mt-1">Analiza sitios web manualmente o programa auditorías automáticas</p>
        </div>
        {audit.isAnalyzed && activeMainTab === 'manual' && (
      ...
      {/* Main tabs: Manual vs Programadas */}
      <Tabs value={activeMainTab} onValueChange={setActiveMainTab}>
        <TabsList className="bg-slate-800/50">
          <TabsTrigger value="manual" className="data-[state=active]:bg-slate-700 data-[state=active]:text-white text-slate-400 gap-2">
            <ScanLine className="h-4 w-4" />
            Auditoría Manual
          </TabsTrigger>
          <TabsTrigger value="scheduled" className="data-[state=active]:bg-slate-700 data-[state=active]:text-white text-slate-400 gap-2">
